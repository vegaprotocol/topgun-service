...
func (s *Service) sortByPartyPositionsMedianFinal(socials map[string]verifier.Social) ([]Participant, error) {
	...
	// Open our jsonFile
	jsonFile3, err := os.Open("day3.json")
	// if we os.Open returns an error then handle it
	if err != nil {
		fmt.Println(err)
	}
	...
	day3Traded := []Participant{}

	// we unmarshal our byteArray which contains our
	// jsonFile's content into 'alreadyTraded' which we defined above
	json.Unmarshal(byteValue3, &day3Traded)
	...
	for _, party := range sParties {
		...
		if (realisedPnL != 0.0) || (unrealisedPnL != 0.0) || (openVolume != 0.0) {
			...
			if PnL != 0 {
				...
				NewTotal = median([]float64{day3Total, day1Total, day2Total})
